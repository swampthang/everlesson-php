...
/**
 * GetProductsApi Class Doc Comment
 *
 * @category Class
 * @package  Everlesson
 * @author   http://github.com/swagger-api/swagger-codegen
...
class GetProductsApi
{
     ...
     * @return \Everlesson\Model\Products
     * @throws \Everlesson\ApiException on non-2xx response
     */
    public function getProducts($token, $membership_id)
    {
        list($response, $statusCode, $httpHeader) = $this->getProductsWithHttpInfo ($token, $membership_id);
        return $response; 
    }
     ...
     * @param string $token token (required)
     * @param string $membership_id membership_id (required)
     * @return Array of \Everlesson\Model\Products, HTTP status code, HTTP response headers (array of strings)
     * @throws \Everlesson\ApiException on non-2xx response
     */
    public function getProductsWithHttpInfo($token, $membership_id)
    {
        
        // verify the required parameter 'token' is set
        if ($token === null) {
            throw new \InvalidArgumentException('Missing the required parameter $token when calling getProducts');
        }
        // verify the required parameter 'membership_id' is set
        if ($membership_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $membership_id when calling getProducts');
        }
  
        // parse inputs
        $resourcePath = "/getProducts";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $headerParams, '\Everlesson\Model\Products'
            );
            ...
            return array(\Everlesson\ObjectSerializer::deserialize($response, '\Everlesson\Model\Products', $httpHeader), $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            case 200:
                $data = \Everlesson\ObjectSerializer::deserialize($e->getResponseBody(), '\Everlesson\Model\Products', $e->getResponseHeaders());
